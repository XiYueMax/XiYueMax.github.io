...
| :---: | :--- | :--- | :--- |
| **`0`** | **关闭验证（免卡密）** | 无需网络验证通信 | 适合单机调试、离线体验或无需卡密限制的场景 |
| **`1`** | **EPIC（摇光云）** | RSA 签名 + Base64 + GZip | 适合需要高安全级 RSA 签名校验的业务 |
| **`2`** | **T3 网络验证** | RC4 加密 + HEX 编码 | 适合功能完善、需要时间戳防重放的成熟系统 |
| **`3`** | **微验（WeiYan）** | RC4-Hex 加解密 | 适合轻量化、快速部署的单码卡密验证 |
